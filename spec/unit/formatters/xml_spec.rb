...
RSpec.describe Chelsea::XMLFormatter do
  it "print_results brings back a Ox xml style object" do
    ...
    formatter = Chelsea::XMLFormatter.new(verbose: true)
    formatter.oi_response = oi_response
    formatter.format_response
    xml = formatter.output
    ...
    expect(xml.testsuite.nodes[0].attributes[:classname])
      .to eq('test')
    expect(xml.testsuite.nodes[0].attributes[:name])
      .to eq('test')
    expect(xml.testsuite.nodes[1].attributes[:classname])
      .to eq('test2')
    expect(xml.testsuite.nodes[1].attributes[:name])
      .to eq('test2')
    expect(xml.testsuite.nodes[2].attributes[:classname])
      .to eq('pkg:npm/js-yaml@1.0.0')
    expect(xml.testsuite.nodes[2].attributes[:name])
      .to eq('pkg:npm/js-yaml@1.0.0')
